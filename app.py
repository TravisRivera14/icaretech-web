...
def listar_usuarios():
    ...
    # SE CORRIGIÓ PARA TRAER TAMBIÉN EL ID DE EMPRESA Y EVITAR ERRORES EN EL FRONTEND
    users_raw = db_query("SELECT id, usuario, rol, nombre, empresa FROM usuarios ORDER BY id ASC", fetch=True) or []
    return jsonify([{"id": r[0], "usuario": r[1], "rol": r[2], "nombre": r[3], "empresa_id": r[4]} for r in users_raw])
...
def editar_usuario():
    ...
    try:
        if d.get('password') and d.get('password').strip() != "":
            db_query("UPDATE usuarios SET usuario = %s, password_hash = %s, nombre = %s, rol = %s, empresa = %s WHERE id = %s", 
                     (d.get('usuario'), generate_password_hash(d.get('password')), d.get('nombre'), d.get('rol'), d.get('empresa_id'), d.get('id')))
        else:
            db_query("UPDATE usuarios SET usuario = %s, nombre = %s, rol = %s, empresa = %s WHERE id = %s", 
                     (d.get('usuario'), d.get('nombre'), d.get('rol'), d.get('empresa_id'), d.get('id')))
...
def crear_ticket():
    ...
    try:
        ...
        db_query("""INSERT INTO tickets_soporte (empresa_id, contacto, asunto, descripcion, prioridad, whatsapp) VALUES (%s, %s, %s, %s, %s, %s)""", (empresa_id, d.get('contacto'), d.get('asunto'), d.get('descripcion'), d.get('prioridad'), d.get('whatsapp')))
...
def eliminar_ticket_admin(id):
    if session.get('rol') not in ['admin', 'personal']: return jsonify({"message": "Denegado"}), 403
    try:
