...
def init_db():
    ...
    # Tabla nueva
    c.execute('''CREATE TABLE IF NOT EXISTS registro_clientes (
        id SERIAL PRIMARY KEY,
        nombre TEXT,
        correo TEXT,
        telefono TEXT,
...
def obtener_todo():
    try:
        ...
        if 'telefono_wa' not in config: db_query("INSERT INTO configuracion (clave, valor) VALUES ('telefono_wa', '50687550729') ON CONFLICT DO NOTHING")
        if 'correo_contacto' not in config: db_query("INSERT INTO configuracion (clave, valor) VALUES ('correo_contacto', 'info@icaretechcr.com') ON CONFLICT DO NOTHING")
        if 'imagen_nosotros' not in config: db_query("INSERT INTO configuracion (clave, valor) VALUES ('imagen_nosotros', 'image_13.png') ON CONFLICT DO NOTHING")
        config_raw = db_query("SELECT clave, valor FROM configuracion", fetch=True) or []
        config = {r[0]: r[1] for r in config_raw}
...
def registro_cliente():
    ...
    try:
        ...
        msg = Message('🔥 Nuevo Cliente Registrado - iCare Tech CR', sender='TU_CORREO@gmail.com', recipients=['info@icaretechcr.com'])
        msg.body = f"Hola Travis, nuevo cliente:\nNombre: {d.get('nombre')}\nCorreo: {d.get('correo')}\nTeléfono: {d.get('telefono')}"
        mail.send(msg)
...
def guardar_config():
    ...
    for k in claves_permitidas:
        if k in d: db_query("INSERT INTO configuracion (clave, valor) VALUES (%s, %s) ON CONFLICT (clave) DO UPDATE SET valor = EXCLUDED.valor", (k, d[k]))
    return jsonify({"mensaje": "✅"})
